...
def rrf_fuse(
    lexical_results: list[tuple[str, float]] | list[ChunkSearchResult],
    vector_results: list[tuple[str, float]] | list[ChunkSearchResult],
    top_k: int = 30,
    k: int = 60,
...
class HybridRetriever:
    ...
    def _vector_results(
        ...
        if query_vector is None:
            return []
        return vector_search_chunks(session, query_vector, plan.hard_constraints, top_k=top_k, query=query)
...
def _group_evidence(results: list[ChunkSearchResult]) -> dict[str, list[ChunkSearchResult]]:
...
def _result_product_id(item: tuple[str, float] | ChunkSearchResult) -> str:
    if isinstance(item, ChunkSearchResult):
        return item.product_id
    return item[0]


def _result_score_bonus(item: tuple[str, float] | ChunkSearchResult) -> float:
    if isinstance(item, ChunkSearchResult):
