...
v2.1 在 v2.0 事实锚定管道的基础上，完成了架构精简并新增了运行稳定性保障。以下测试查询可快速验证当前版本能力：
...
### v2.1 架构精简（无功能变化，体验更稳定）

- **代码清理**：删除 `tool_plan.py`、`intent_compiler.py` 及 `UnifiedPlan` 兼容属性，净删除约 1350 行死代码，架构更清晰。
- **目录重组**：`server/backend/app/` 按职责划分为 `core/`、`planning/`、`pipeline/`、`retrieval/`、`rag/`、`tools/`、`services/`、`repositories/`、`db/` 等子目录，新增模块定位更直观。
- **阶段超时**：plan / retrieve / generate / tool 各阶段独立超时，单点慢请求不会无限阻塞。
- **LLM 信号量**：限制并发 LLM 调用数，突发请求下服务更稳定。
- **工具错误分类**：工具异常按阶段分类，降级提示更精确（如"检索阶段超时，请稍后重试"而非统一"服务不可用"）。
...
v2.1 会按 `plan` / `retrieve` / `generate` / `tool` / `internal` 分类错误来源，日志中会有对应的 `failure_stage` 标记。
...
plan_tool + stream_response 两轮 LLM 调用合计 20-40s 是正常范围。观察 `/dev` 仪表盘的延时数据来确认瓶颈。v2.1 的阶段超时会在瓶颈过久时主动触发降级，避免无反馈等待。
...
- [ ] curl 发 "推荐一款6000元左右的小米手机" → 得到品牌约束推荐
- [ ] curl 发 "分析一下小米 17Max的优缺点" → 得到产品分析回复
- [ ] curl 发 "有没有便宜一点的替代品？"（接推荐对话后） → 得到更便宜替代品
...
- `docs/design.md` — 系统架构与技术细节
- `docs/CHANGELOG.md` — v2.0 / v2.1 升级说明
