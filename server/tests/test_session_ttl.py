...
class TestSessionStoreTTL:
    def test_cleanup_expired_deletes_old_files(self, tmp_path):
        ...
        user_dir = persist_dir / "anonymous"
        user_dir.mkdir()

        old_file = user_dir / "old_session.json"
        old_file.write_text(
            SessionContext(session_id="old_session").model_dump_json(), encoding="utf-8"
        )

        new_file = user_dir / "new_session.json"
        new_file.write_text(
        ...
        assert ("anonymous", "new_session") in store._sessions
        assert ("anonymous", "old_session") not in store._sessions

    def test_atomic_write_uses_tmp_then_rename(self, tmp_path):
        persist_dir = tmp_path / "sessions"
        store = SessionStore(persist_dir=str(persist_dir), ttl_days=7)
        ctx = SessionContext(session_id="s1")
        store._sessions[("anonymous", "s1")] = ctx
        store.save("anonymous", "s1")

        assert (persist_dir / "anonymous" / "s1.json").exists()
        assert not (persist_dir / "anonymous" / "s1.tmp").exists()
    ...
    def test_corruption_recovery_moves_to_corrupted(self, tmp_path):
        ...
        user_dir = persist_dir / "anonymous"
        user_dir.mkdir()
        bad_file = user_dir / "bad.json"
        bad_file.write_text("not valid json", encoding="utf-8")

        store = SessionStore(persist_dir=str(persist_dir), ttl_days=7)
        result = store._load_one("anonymous", "bad")

        assert result is None
        assert (user_dir / "bad.corrupted").exists()
    ...
    def test_schema_version_set_on_save(self, tmp_path):
        ...
        loaded = store._load_one("anonymous", "s1")
    ...
    def test_migrate_called_for_old_schema_version(self, tmp_path):
        ...
        path = user_dir / "s1.json"
        path.write_text(ctx.model_dump_json(), encoding="utf-8")

        store = SessionStore(persist_dir=str(persist_dir), ttl_days=7)
        loaded = store.get("anonymous", "s1")
        assert loaded.schema_version == SessionStore.CURRENT_SCHEMA_VERSION
    ...
    def test_save_all_persists_all_sessions(self, tmp_path):
        persist_dir = tmp_path / "sessions"
        store = SessionStore(persist_dir=str(persist_dir), ttl_days=7)
        store._sessions[("anonymous", "s1")] = SessionContext(session_id="s1")
        store._sessions[("anonymous", "s2")] = SessionContext(session_id="s2")
        store.save_all()

        assert (persist_dir / "anonymous" / "s1.json").exists()
        assert (persist_dir / "anonymous" / "s2.json").exists()
