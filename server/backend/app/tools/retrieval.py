    ...
    async def execute(self, request: ChatRequest, context: SessionContext, **kwargs) -> AsyncIterator[dict]:
        ...
        if memory_hit is not None:
            async for event in self._agent._stream_recommendation_events(
                user_id, request, plan, memory_hit.ranked, context_action,
                memory_mode=memory_hit.mode, selected_override=memory_hit.ranked, cached_summary=memory_hit.summary,
            ):
                yield event
            return
        ranked = await self._agent.retrieve_and_rank(plan, user_id=user_id, session_id=request.session_id)
        async for event in self._agent._stream_recommendation_events(user_id, request, plan, ranked, context_action):
            yield event
