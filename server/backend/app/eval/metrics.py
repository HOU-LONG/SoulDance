...
def _check_brands(
    ...
    for expected in expected_brands:
        if not any(expected.lower() in brand for brand in returned_lower):
            failures.append(f"missing expected brand: {expected}")
    for forbidden in forbidden_brands:
        if any(forbidden.lower() in brand or brand in forbidden.lower() for brand in returned_lower):
            failures.append(f"forbidden brand returned: {forbidden}")
...
def _check_product_subset(failures: list[str], product_ids: list[str], whitelist: list[str]) -> None:
    ...
    for pid in product_ids:
        if pid not in allowed:
            failures.append(f"product {pid} outside expected subset")
