...
1. 独立 `session_id`：`eval_c{N}_2026-06-24`
2. 独立 cache namespace：`data/eval/long_session_2026-06-24/cache_c{N}/`
3. **每个 condition 独立进程**，CLI 跑完即退出，避免内存 dict 残留
4. runner 启动时 `--reset-cache` 强制清空对应目录，trace 起始记录 cache stats，必须为全 0
5. 跨 condition 严格串行，不并发
...
- **would_hit_rate**：假设 `.get()` 未禁用，本轮命中率（runner 即使禁用 get 也并行计算一次，仅用于统计，不影响业务路径）
...
- **预算估算**：约 150-250 次 judge × 3 = 450-750 次额外 ARK 调用
...
```jsonc
{
  ...
  "cache_namespace": "data/eval/long_session_2026-06-24/cache_c2/",
...
Stage 1 — dry-run (每 condition 20 轮 = 100 轮)
  ...
  - trace 字段完整（所有字段非缺失）
  - 缓存隔离验证：每个 condition 起始 cache stats 都是 0
  - 断点续跑测试：手动 kill 一次，重启从下一轮继续，trace 不重不漏
  - ARK 真实成本估算：把 100 轮成本 × 55 推估全量成本，写入 spec 附录
  - 产出 dry-run 摘要 markdown 给 user review
  - 落在 data/eval/long_session_2026-06-24/dryrun/，不污染 full run 目录
...
- **总轮次**：5 × 1100 = 5500 轮
- **ARK 调用估算**：每轮均 ~2 次 ARK 调用（planner + answer）+ judge 约 450-750 次 → **~12000-14000 次 ARK 调用**
- **单轮均耗** ~2.5 s，串行总时长 ≈ **3.8 小时**
- **ARK 成本**：粗估总 token ~20-25M，按豆包定价约 **100-200 元区间**；dry-run 后用 100 轮真实数据 × 55 推估，写入 spec 附录后再放行 full run
- **关键风险**：
  - 长 session ARK 限流 → 接 §8 续跑协议
  - C0 在某轮后必然触发硬截断 → 接 §3.1 解释口径
  - LLM judge 自身波动 → 3 次平均 + temperature=0
  - 缓存污染 → 接 §3.3 + 独立进程
...
│   ├── judge_C0..C4.csv
│   └── REPORT.md
├── cache_c0/ ... cache_c4/                # 各 condition 隔离 cache
└── plots/
...
- `server/backend/app/memory_cache.py`
  ...
  - `.put()` 不受开关影响（便于 would_hit 统计）
- `server/backend/app/agent.py`：调用 `.get()` 时透传开关；并行计算 `would_hit_b1/b2` 落 trace
...
5. **缓存独立**：5 condition × 独立 session_id × 独立 cache 目录 × 独立进程
6. **状态机不动**：禁用开关只切"注入/使用"，不切"维护"
7. **命中率双口径**：would_hit / effective_hit 区分
8. **指标按 turn type 分桶**：不用 NDCG 一把梭
9. **对抗轮内置**：75 轮含糊指代/矛盾约束/跨类目切换/撤销/错指代攻击
10. **LLM judge 受限**：仅 4 个 turn type 采样，3 次平均，rubric 固化版本化
11. **断点续跑 hash 校验**：模板/商品/condition 变更后拒绝续跑
12. **Pilot Gate**：dry-run → pilot → full，每级 user-approved
13. **成本可观测**：dry-run 后真实推估，写入 spec 附录再放行 full run
