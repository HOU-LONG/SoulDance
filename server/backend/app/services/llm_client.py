...
from ..retrieval.knowledge_base import evidence_review_summary
...
_prompts = PromptRegistry(Path(__file__).parent.parent / "prompts")
...
PRODUCT_ANALYSIS_SYSTEM_PROMPT = (
    ...
    "用户想了解一款具体商品的价值。\n\n"
    "重要规则：\n"
    "1. 你只能分析本店商品库中实际存在的商品。如果库中没有该商品，直接告知用户。\n"
    "2. 绝对禁止说「该产品不存在于市场」「该型号未发布」——你只代表本店库存，不代表品牌方。\n"
    "3. 基于用户消息中附带的商品事实信息进行分析，不要引用你训练数据中的外部知识。\n"
    "4. 从性价比、适用场景、优缺点角度给出简短分析（2-4句话）。\n"
    "5. 不要编造价格和规格。"
)

# 未在本店库存命中时的回答——绝不替用户判断产品"是否真实存在"，只说明库内检索结果
PRODUCT_ANALYSIS_UNKNOWN_SYSTEM_PROMPT = (
    "你是本店 AI 助手灵舞。用户问到一款具体商品（品牌+型号），"
    "但本店商品库中未匹配到此商品。\n\n"
    "核心规则：\n"
    "1. 绝对禁止说「该产品不存在」「该型号未发布」「这是虚构产品」等否定产品存在的表述——"
    "你只代表本店的商品库，不代表整个市场或品牌方。\n"
    "2. 正确说法：「本店商品库中暂未找到该商品，请确认型号是否准确，或换一个搜索词试试。」\n"
    "3. 如果用户坚持这个型号，引导用户描述需求（预算、用途、偏好），从库中推荐相近商品。\n"
    "4. 不要引用任何你训练数据中的公开信息来「分析」这款商品——既然库中没有，分析也是无源之水。\n"
    "回答风格：直接、诚实、有帮助，2-3 句话。末尾可轻量引导用户描述需求以便推荐库内商品。"
)
...
class DoubaoLLMClient:
    ...
    async def generate_summary(self, history_text: str) -> str:
        ...
        prompt_path = Path(__file__).parent.parent / "prompts" / "v1" / "summary.txt"
